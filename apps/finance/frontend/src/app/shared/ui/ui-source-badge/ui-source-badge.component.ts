...
const DEFAULT_TITLES: Record<UiSourceKind, string> = {
  observed: 'Net Worth — current balances, not a transaction rollup',
  scheduled: 'Cashflow — recurring scheduled money, does not change net worth',
  combined: 'Activity + Cashflow for the selected period, not net worth',
  scenario: 'Planning — speculative, does not mutate truth',
};

@Component({
  selector: 'ui-source-badge',
  standalone: true,
  template: `<span [class]="hostClass()" [attr.title]="resolvedTitle()">{{ label() }}</span>`,
  styles: [
    `
      .ui-source-badge {
        display: inline-flex;
...
export class UiSourceBadgeComponent {
  ...
  resolvedTitle = computed(() => this.title() || DEFAULT_TITLES[this.kind()]);
